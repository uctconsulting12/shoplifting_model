...
from analyzer import EnhancedKeypointAnalyzerV4, BehaviorAnalyzer
from detector import PersonDetectorTracker, EnhancedContainerTracker, ItemTracker
'''
    ...
    def get_or_create_camera_state(self, cam_id: int) -> Dict:
        ...
        if cam_id not in self.camera_states:
            self.camera_states[cam_id] = {
                ...
                'person_alert': {},
                'ever_alerted_persons': set()  # Track persons who have ever been alerted
            }
...
def _analyze_behaviors(model: TheftDetectionModel,
    ...
    ever_alerted_persons = cam_state['ever_alerted_persons']
            ...
            if alert_level.value >= AlertLevel.ATTENTION.value:
                ...
                if alert_level != state.last_alert_level or alert_level == AlertLevel.ALERT:
                    alert = _generate_alert(
                        ...
                        person_alert, ever_alerted_persons
                    )
                    alerts.append(alert)
...
def _generate_alert(person_id: int,
                    ...
                    behavior_analyzer: BehaviorAnalyzer,
                    person_alert: Dict,
                    ever_alerted_persons: set = None) -> Dict:
    ...
    if base_message != "✅ Normal" and not person_alert[person_id]:  # ← Changed logic
        ...
        # Mark this person as ever alerted (for permanent red color)
        if ever_alerted_persons is not None:
            ever_alerted_persons.add(person_id)
    else:
...
def _visualize_frame(frame: np.ndarray,
    ...
    for person_id, bbox in tracked_persons.items():
        try:
            ...
            # IMPORTANT: Once a person has been alerted, they stay RED permanently
            if person_id in ever_alerted_persons:
                # Person has been alerted before - keep them RED permanently
                color = (0, 0, 255)  # Red
                thickness = 3
            elif person_id in alert_map:
